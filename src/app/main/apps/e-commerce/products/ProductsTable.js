...
function ProductsTable(props) {
  ...
  useEffect(() => {
    if (searchText.length !== 0) {
      setData(
        _.filter(hotels, (item) => item.name.toLowerCase().includes(searchText.toLowerCase()))
      );
      setPage(0);
    } else {
      setData(hotels);
  ...
  function handleClick(item) {
    props.navigate(`/apps/e-commerce/products/${item._id}/${item.handle}`);
  }
